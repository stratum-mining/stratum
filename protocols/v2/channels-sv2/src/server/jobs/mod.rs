...
/// Describes the origin of a mining job (template or custom job message).
#[derive(Clone, Debug, PartialEq)]
pub enum JobOrigin<'a> {
    /// The job was created from a `NewTemplate` message.
    NewTemplate(NewTemplate<'a>),
    /// The job was created from a `SetCustomMiningJob` message.
