...
//! Stratum V2 (SV2) standard channel as maintained on a mining pool server.
//!
//! A standard channel represents a direct mining relationship between a pool and a single
//! downstream mining entity (device or proxy) using SV2. The channel tracks the parameters
//! negotiated at channel opening, manages job lifecycle and activation, tracks share submissions,
//! and performs share validation and accounting according to SV2 protocol rules.
//!
//! ## Responsibilities
//!
...
impl<'a> StandardChannel<'a> {
    /// Constructs a new `StandardChannel` instance.
    ///
    /// Initializes the channel state with negotiated parameters, accounting, and job management.
    /// Returns an error if the target difficulty is invalid or violates the requested max target.
    ...
    /// Updates the nominal hashrate and/or requested max target.
    ///
    /// Recalculates the channel's target difficulty.
    /// Returns an error if the new target is invalid or exceeds the requested max target.
