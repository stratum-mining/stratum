...
pub enum CError {
    BadSerdeJson,
    /// Errors on bad conversion from system time to UNIX timestamp.
    BadSystemTimeFromTimestamp,
    /// Errors on bad conversion from UNIX timestamp to system time.
    BadTimestampFromSystemTime(u32),
    /// Errors from the `binary_sv2` crate
    // BinarySv2Error(binary_sv2::CError),
    // RR TODO: Can I use binary_sv2::CError?
    BinarySv2Error,
    Bs58DecodeError,
    /// Errors on expired certificate. Displays validity expiration time and current time:
    /// `(not_valid_after, now)`.
...
impl From<Error> for CError {
    fn from(e: Error) -> CError {
        match e {
            Error::BadSerdeJson(_) => CError::BadSerdeJson,
            Error::BadSystemTimeFromTimestamp(_) => CError::BadSystemTimeFromTimestamp,
            Error::BadTimestampFromSystemTime(u) => CError::BadTimestampFromSystemTime(u),
            Error::BinarySv2Error(_) => CError::BinarySv2Error,
            Error::Bs58DecodeError(_) => CError::Bs58DecodeError,
            Error::CertificateExpired(u1, u2) => CError::CertificateExpired(u1, u2),
            Error::CertificateInvalid(u1, u2) => CError::CertificateInvalid(u1, u2),
            Error::DalekError(_) => CError::DalekError,
            Error::EncryptionAlgorithmInvalid(u) => CError::EncryptionAlgorithmInvalid(u),
            Error::EncryptionAlgorithmNotFound => CError::EncryptionAlgorithmNotFound,
            Error::ExpectedIncomingHandshakeMessage => CError::ExpectedIncomingHandshakeMessage,
            Error::HSInitiatorStepNotFound(u) => CError::HSInitiatorStepNotFound(u),
            Error::HSResponderStepNotFound(u) => CError::HSResponderStepNotFound(u),
            Error::IoError => CError::IoError,
            Error::MessageToDecryptIsEmpty => CError::MessageToDecryptIsEmpty,
            Error::MustSpecifyOneEncryptionAlgorithm(u) => {
                CError::MustSpecifyOneEncryptionAlgorithm(u)
            }
            Error::SnowError(_) => CError::SnowError,
            Error::SnowNoRemoteStaticKey => CError::SnowNoRemoteStaticKey,
            Error::NoiseTodo => CError::NoiseTodo,
        }
    }
}
...
impl Drop for CError {
    fn drop(&mut self) {
        match self {
            CError::BadSerdeJson => (),
            CError::BadSystemTimeFromTimestamp => (),
            CError::BadTimestampFromSystemTime(_) => (),
            CError::BinarySv2Error => (),
            CError::Bs58DecodeError => (),
            CError::CertificateExpired(_, _) => (),
            CError::CertificateInvalid(_, _) => (),
            CError::DalekError => (),
            CError::EncryptionAlgorithmInvalid(_) => (),
            CError::EncryptionAlgorithmNotFound => (),
            CError::ExpectedIncomingHandshakeMessage => (),
            CError::HSInitiatorStepNotFound(_) => (),
            CError::HSResponderStepNotFound(_) => (),
            CError::IoError => (),
            CError::MessageToDecryptIsEmpty => (),
            CError::MustSpecifyOneEncryptionAlgorithm(_) => (),
            CError::SnowError => (),
            CError::SnowNoRemoteStaticKey => (),
            CError::NoiseTodo => (),
        }
    }
}
