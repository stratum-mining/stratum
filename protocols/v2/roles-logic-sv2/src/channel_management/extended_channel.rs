...
use crate::{
    job_management::{JobFactory, JobFactoryError},
...
/// An Extended Channel receives Extended Jobs.
///
/// An Extended Channel keeps track of one current, one future, and few multiple past jobs.
///
...
///    servers.
/// 2. Receiving `NewExtendedMiningJob` messages and pushing them to the Channel State. Useful for
///    mining clients.
///
/// An Extended Channel keeps track of the share submission process for current and past jobs.
///
...
impl ExtendedChannel {
    pub fn new(
        ...
        Self {
            ...
            nominal_hashrate,
            job_factory: JobFactory::new(extranonce_size as u8),
    ...
    fn push_past_job(&mut self, job: NewExtendedMiningJob<'static>) {
    ...
    pub fn set_extranonce_size(&mut self, extranonce_size: u16) {
    ...
    ///
    /// This method is meant for users that are not using the JobFactory to create jobs,
    /// but instead are receiving `NewExtendedMiningJob` messages and need to push them to the
    /// Channel State.
    pub fn push_active_job(&mut self, job: NewExtendedMiningJob<'static>) {
        self.push_past_job(job.clone());
    ...
    /// but instead are receiving `NewExtendedMiningJob` messages and need to push them to the
    /// Channel State.
    pub fn set_future_job(&mut self, job: NewExtendedMiningJob<'static>) {
    ...
    ///
    /// And we return the `NewExtendedMiningJob` message that can be sent to the network.
    pub fn new_active_job(
        &mut self,
    ) -> Result<NewExtendedMiningJob<'static>, ExtendedChannelError> {
        let job = self
            .job_factory
            .extended_job_from_current_template()
    ...
    pub fn new_future_job(
        ...
        let job = self
            .job_factory
            .extended_job_from_future_template()
            .map_err(|_| ExtendedChannelError::JobFactoryError)?;
    ...
    pub fn validate_share(
    ...
    ) -> Result<bool, ExtendedChannelError> {
        ...
        if !is_active_job && !is_past_job {
            return Err(ExtendedChannelError::ShareHasInvalidJobId);
        }
        ...
        .ok_or(ExtendedChannelError::InvalidShare)?
        .try_into()
        .expect("merkle root must be 32 bytes");

        // get the previous hash and nbits from channel state
        let prev_hash = self
            .active_prev_hash
            .as_ref()
            .ok_or(ExtendedChannelError::NoPrevHash)?;
        let nbits = self.active_nbits.ok_or(ExtendedChannelError::NoNbits)?;
        ...
        if hash <= self.target {
            ...
            Err(ExtendedChannelError::ShareDoesNotMeetTarget)
        }
    }
