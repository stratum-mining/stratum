...
use mining_sv2::{
    ...
    SubmitSharesError, SubmitSharesExtended, SubmitSharesStandard,
};
...
impl ChannelFactory {
    ...
    fn check_target<TxHash: std::convert::AsRef<[u8]>>(
        ...
        let upstream_target = match &self.kind {
            ExtendedChannelKind::Pool => Target::ZERO,
        ...
        {
            let bitcoin_target_log = bitcoin_target.to_be_bytes();
            debug!("Bitcoin target : {:?}", bitcoin_target_log.as_hex());
            let upstream_target = upstream_target.to_be_bytes();
            debug!("Upstream target: {:?}", upstream_target.to_vec().as_hex());
            let mut hash = hash;
            hash.reverse();
            debug!("Hash           : {:?}", hash.to_vec().as_hex());
        }
        let hash = Target::from_be_bytes(hash);
    ...
    fn get_channel_specific_mining_info(&self, m: &Share) -> Option<(Target, Vec<u8>)> {
        match m {
            Share::Extended(share) => {
                let channel = self.extended_channels.get(&m.get_channel_id())?;
                let extranonce_prefix = channel.extranonce_prefix.to_vec();
                let dowstream_target =
                    Target::from_le_bytes(channel.target.inner_as_ref().try_into().unwrap());
    ...
    fn update_target_for_channel(&mut self, channel_id: u32, new_target: Target) -> Option<bool> {
        ...
        channel.target = new_target.to_le_bytes().into();
        Some(true)
    }
}
...
impl ProxyExtendedChannelFactory {
    ...
    pub fn on_submit_shares_extended(
        ...
        if let Some(job_creator) = self.job_creator.as_mut() {
            ...
            let bitcoin_target = Target::ZERO;
