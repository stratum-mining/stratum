...
//! ensure secure communication. To encode messages for transmission, choose between the `Encoder`
//! for standard Sv2 frames or the `NoiseEncoder` for encrypted frames. To decode received
//! messages, choose between the `StandardDecoder` for standard Sv2 frames or
...
///
/// The state transitions from initialization (`NotInitialized`) to handshake (`HandShake`)
/// and finally to transport mode (`Transport`) as the encryption handshake is completed.
#[cfg(feature = "noise_sv2")]
#[derive(Debug)]
#[allow(clippy::large_enum_variant)]
pub enum State {
...
impl State {
    ...
    ///
    /// nb: This method returns a `HandShakeFrame` but does not change the current state (`self`).
    /// The state remains `HandShake(Initiator)` until `step_1` is called to advance the handshake
    /// process.
    ...
    /// enables full encryption and decryption in `Transport` mode.
    ///
...
impl State {
    ...
    ///
    /// Sets the codec to the initial state, `NotInitialized`, based on the provided handshake
    /// role. This state is used before the handshake process begins, and the handshake message
    /// size guides the codec on how much data to expect before advancing to the next step. The
    /// expected size of the handshake message is determined by whether the codec is acting as an
    ...
    /// in `HandShake` mode, the codec begins negotiating cryptographic keys with the peer,
    /// eventually transitioning to the secure `Transport` phase.
    ///
    ...
    ///
    /// Finalizes the handshake process and transitions the codec into `Transport` mode, where full
    /// encryption and decryption are active. The codec uses the provided `NoiseCodec` to perform
    /// encryption and decryption for all communication in this mode, ensuring secure data
    /// transmission.
    ///
    /// Once in `Transport` mode, the codec is fully operational for secure communication.
