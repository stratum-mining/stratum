...
use super::error::{PoolError, PoolErrorBranch};
...
async fn send_status(sender: &Sender, e: PoolError, outcome: PoolErrorBranch) -> PoolErrorBranch {
    match sender {
...
pub async fn handle_error(sender: &Sender, e: PoolError) -> PoolErrorBranch {
    tracing::debug!("Error: {:?}", &e);
    match e {
        PoolError::Io(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::ChannelSend(_) => {
            ...
            send_status(sender, e, PoolErrorBranch::Continue).await
        }
        PoolError::ChannelRecv(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::BinarySv2(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::Codec(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::Noise(_) => send_status(sender, e, PoolErrorBranch::Continue).await,
        PoolError::RolesLogic(roles_logic_sv2::Error::NoDownstreamsConnected) => {
            send_status(sender, e, PoolErrorBranch::Continue).await
        }
        PoolError::RolesLogic(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::Custom(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::Framing(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::PoisonLock(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::ComponentShutdown(_) => send_status(sender, e, PoolErrorBranch::Break).await,
        PoolError::Sv2ProtocolError(_) => send_status(sender, e, PoolErrorBranch::Break).await,
    }
}
