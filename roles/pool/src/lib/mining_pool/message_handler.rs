...
use stratum_common::roles_logic_sv2::{
    bitcoin::{consensus::Decodable, transaction::TxOut, Amount, Target},
    channels_sv2::server::{
...
impl ParseMiningMessagesFromDownstream<()> for Downstream {
    ...
    fn handle_open_extended_mining_channel(
        ...
        let requested_max_target =
            Target::from_le_bytes(m.max_target.inner_as_ref().try_into().unwrap());
    ...
    fn handle_update_channel(&mut self, m: UpdateChannel) -> Result<SendTo<()>, Error> {
        ...
        let requested_maximum_target =
            Target::from_le_bytes(m.maximum_target.inner_as_ref().try_into().unwrap());
        ...
        if is_standard_channel {
            ...
            let set_target = SetTarget {
                channel_id,
                maximum_target: new_target.clone().to_le_bytes().into(),
            };
            Ok(SendTo::Respond(Mining::SetTarget(set_target)))
        } else {
