...
impl Device {
    async fn start(
        ...
        info!("Mining device started");
        loop {
            let incoming = receiver.recv().await.unwrap();
            let mut incoming: StdFrame = incoming.try_into().unwrap();
            let message_type = incoming.get_header().unwrap().msg_type();
            let payload = incoming.payload();
            ...
            let msg: Result<MiningDeviceMessages, _> = (message_type, payload).try_into();
            info!("Mining device received message: {:?}", msg);
