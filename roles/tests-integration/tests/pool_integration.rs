...
use const_sv2::{
    MESSAGE_TYPE_NEW_EXTENDED_MINING_JOB, MESSAGE_TYPE_NEW_TEMPLATE, MESSAGE_TYPE_SETUP_CONNECTION,
    MESSAGE_TYPE_SETUP_CONNECTION_ERROR, MESSAGE_TYPE_SET_NEW_PREV_HASH,
...
use std::{alloc::System, convert::TryInto, time::SystemTime};
...
async fn header_timestamp_value_assertion_in_new_extended_mining_job() {
    ...
    let tp_pool_sniffer =
        common::start_sniffer("0".to_string(), tp_pool_sniffer_addr, tp_addr, false, None).await;
    ...
    println!(
        "Header timestamp value sent by Template Provider: {}",
        header_timestamp_to_check
    );

    // Verify messages between Pool and Translator Proxy
    assert_common_message!(
    ...
    println!("Second min_ntime value: {:?}", second_job_ntime);
