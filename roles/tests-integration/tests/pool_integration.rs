...
async fn pool_standard_channel_receives_share() {
    ...
    // for some reason, the pool sends a SetNewPrevHash message before the
    // OpenStandardMiningChannelSuccess message so for now, we leave this commented out
    // sniffer.wait_for_message_type(MessageDirection::ToDownstream,
    // MESSAGE_TYPE_SET_NEW_PREV_HASH).await;
