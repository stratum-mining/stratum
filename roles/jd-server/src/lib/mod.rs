...
impl JobDeclaratorServer {
    ...
    /// Bitcoin node. An async job is then started in order to update the mempool at regular intervals.
    /// After that, JDS will start a TCP server to listen for incoming connections from JDC(s).
    ///
