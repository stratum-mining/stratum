...
impl Downstream {
    ...
    ) -> std::result::Result<(), super::super::error::Error<'static>> {
...
impl IsServer<'static> for Downstream {
    ...
    fn handle_configure(
        ...
        (
            ...
            ).expect("Version mask invalid, automatic version mask selection not supported, please change it in carte::downstream::mod.rs")),
            Some(false),
        )
    }
    ...
    fn handle_subscribe(&self, request: &client_to_server::Subscribe) -> Vec<(String, String)> {
        ...
        let set_difficulty_sub = (
            ...
            downstream::new_subscription_id(),
        );
        let notify_sub = (
