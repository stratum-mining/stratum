...
fn process_cli_args<'a>() -> ProxyResult<'a, TranslatorProxyConfig> {
    // Parse CLI arguments
    ...
    // Deserialize settings into TranslatorProxyConfig
    let config = settings.try_deserialize::<TranslatorProxyConfig>()?;
    Ok(config)
}
