...
mod tests {
    ...
    fn create_test_difficulty_config() -> DownstreamDifficultyConfig {
        DownstreamDifficultyConfig::new(100.0, 5.0, true, 0, 0)
    }

    fn create_test_downstream_config() -> DownstreamConfig {
        DownstreamConfig::new("0.0.0.0".to_string(), 3333, create_test_difficulty_config())
    }
    ...
    fn test_downstream_difficulty_config_creation() {
        ...
        assert_eq!(config.submits_since_last_update, 0);
        assert_eq!(config.timestamp_of_last_update, 0);
    }

    #[test]
    fn test_downstream_difficulty_config_equality() {
        let config1 = DownstreamDifficultyConfig::new(100.0, 5.0, true, 0, 0);
        let config2 = DownstreamDifficultyConfig::new(100.4, 5.0, false, 10, 12345);
        assert_eq!(config1, config2); // Only hashrate is compared, rounded

        let config3 = DownstreamDifficultyConfig::new(101.0, 5.0, true, 0, 0);
        assert_ne!(config1, config3);
    }

    #[test]
    fn test_downstream_config_creation() {
        let config = create_test_downstream_config();
    ...
    fn test_translator_config_creation() {
        ...
        let downstream = create_test_downstream_config();
    ...
    fn test_vardiff_disabled_config() {
        ...
        let downstream = DownstreamConfig::new("0.0.0.0".to_string(), 3333, difficulty_config);
