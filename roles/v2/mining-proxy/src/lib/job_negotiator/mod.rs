...
use codec_sv2::Frame;
use roles_logic_sv2::{handlers::job_negotiation::ParseServerJobNegotiationMessages, template_distribution_sv2::NewTemplate};
...
impl JobNegotiator {
    pub async fn new(address: SocketAddr, authority_public_key: [u8; 32], receiver_new_template: Receiver<NewTemplate<'static>>,) {
    ...
    pub fn on_new_template(self_mutex: Arc<Mutex<Self>>) {
        task::spawn(async move {
            loop {
                let receiver_new_tp = self_mutex.clone().safe_lock(|d| d.receiver_new_template.clone()).unwrap();
                let incoming_new_template: NewTemplate = receiver_new_tp.recv().await.unwrap().try_into().unwrap();
                println!("New template {:?}", incoming_new_template);
                self_mutex.safe_lock(|t| {t.last_new_template = Some(incoming_new_template);});
            }
        });
    }
