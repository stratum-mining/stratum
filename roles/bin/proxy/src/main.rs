...
async fn main() {
    ...
    proxy_sv2::ROUTING_LOGIC
        .set(Mutex::new(
            proxy_sv2::initialize_r_logic(&config.upstreams, group_id, config.clone()).await,
        ))
    ...
    info!("PROXY INITIALIZED");
    proxy_sv2::downstream_mining::listen_for_downstream_mining(socket).await
}
