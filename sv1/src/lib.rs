...
pub trait IsServer<'a> {
    ...
    /// jobs.
    ///
    /// The client can specify `mining.notify` `job_id` the client wishes to resume working with
    ///
...
pub trait IsClient<'a> {
    /// Deserialize a raw json_rpc message into a stratum v1 message and handle the
    /// result.
    fn handle_message(
        &mut self,
