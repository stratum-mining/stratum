...
#[path = "./lib/client.rs"]
mod client;
use crate::client::{Client, extranonce_from_hex};
...
fn get_authorize() {
    let mut client = Client::new(0);
    client.status = ClientStatus::Configured;
    black_box(client.authorize(black_box(10),black_box("user".to_string()),black_box("passowrd".to_string())).unwrap());
}
...
fn get_submit() {
    let mut client = Client::new(0);
    client.status = ClientStatus::Configured;
    black_box( client.submit(0, "user".to_string(), extranonce_from_hex("00"), 78, 78, None).unwrap());
}
