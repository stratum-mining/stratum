...
#[path = "./lib/client.rs"]
mod client;
use crate::client::{Device, EitherFrame};
...
async fn share_submit() {
    async_std::task::block_on(async {
        let addr: SocketAddr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(0, 0, 0, 0)), 34254);
        let stream = TcpStream::connect(addr).await.unwrap();
        let (receiver, sender): (Receiver<EitherFrame>, Sender<EitherFrame>) = Connection::new(
            stream,
            HandshakeRole::Initiator(Initiator::from_raw_k(AUTHORITY_PUBLIC_K).unwrap()),
            10,
        )
        .await;
