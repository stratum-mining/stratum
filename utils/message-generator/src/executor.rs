...
use crate::net::{setup_as_upstream,setup_as_downstream};
    ...
    pub async fn new(test: Test<Message>) -> Self {
        match (test.as_dowstream, test.as_upstream) {
            (Some(as_down), Some(as_up)) => {
                let (recv_from_down,send_to_down) = setup_as_upstream(as_up.addr, as_up.keys).await;
    ...
    pub async fn execute(self) -> bool {
        for action in self.actions {
            let (sender,receiver) = match action.role {
                Role::Upstream => {
                    (self.send_to_down.as_ref().expect("Action require executor to act as upstream")
                    ,self.recv_from_down.as_ref().expect("Action require executor to act as upstream")
                    )
                },
                Role::Downstream => {
                    (self.send_to_up.as_ref().expect("Action require executor to act as downstream")
                    ,self.recv_from_up.as_ref().expect("Action require executor to act as downstream")
                    )
                }
            };
