...
//! through owned memory ([`Vec<u8>`]) or externally managed memory in a buffer pool
//! ([`crate::BufferPool`]).
//!
...
//!   allocations.
//! - **Shared Access**: Allows safe concurrent access using atomic state tracking
//!   ([`Arc<AtomicU8>`]).
...
//!    from a buffer pool ([`crate::BufferPool`]), reducing dynamic memory allocation.
//!
//! ### Debug Mode
...
/// A special index value used to mark [`Slice`] as ignored in certain operations, such as memory
/// pool tracking or state management.
///
/// It can be used as a sentinel value for slices that should not be processed or tracked, helping
/// differentiate valid slices from those that need to be skipped. When a [`Slice`]'s `index` is
/// set to `INGORE_INDEX`, it is flagged to be ignored and by any logic that processes or tracks
/// slice indices.
pub const INGORE_INDEX: u8 = 59;

// Allows `Slice` to be safely transferred between threads.
//
// `Slice` contains a raw pointer (`*mut u8`), so Rut cannot automatically implement `Send`. The
// `unsafe` block asserts that memory access is thread-safe, relaying on `SharedState` and atomic
// operations to prevent data races.
unsafe impl Send for Slice {}

/// A contiguous block of memory, either preallocated for dynamically allocated.
///
...
pub struct Slice {
    ...
    ///
    /// Allows for efficient access to the underlying memory. Care should be taken when working
    /// with raw pointers to avoid memory safety issues. The pointer must be valid and must point
    /// to a properly allocated and initialized memory region.
    ...
    ///
    /// Tracks the slice within the pool and manages memory reuse. It allows for quick
    /// identification of slices when freeing or reassigning memory.
    pub index: u8,

    /// Shared state of the memory pool.
    ///
    /// Tracks how many slices are currently in use and ensures proper synchronization of memory
    /// access across multiple contexts.
    ...
    /// [`None`], the slice points to memory managed by the memory pool.
...
///
/// Encapsulates an atomic 8-bit value ([`AtomicU8`]) to track the shared state of memory slices
/// in a thread-safe manner. It uses atomic operations to ensure that memory tracking can be
/// done concurrently without locks.
///
/// Each bit in the [`AtomicU8`] represents the state of a memory slot (e.g., whether it is
/// allocated or free) in the buffer pool, allowing the system to manage and synchronize memory
/// usage across multiple slices.
///
/// [`SharedState`] acts like a reference counter, helping the buffer pool know when a buffer slice
/// is safe to clear. Each time a memory slice is used or released, the corresponding bit in the
/// shared state is toggled. When no slices are in use (all bits are zero), the buffer pool can
/// safely reclaim or reuse the memory.
///
...
impl Default for SharedState {
    /// Creates a new [`SharedState`] with an internal [`AtomicU8`] initialized to `0`, indicating
    /// no memory slots are in use.
    fn default() -> Self {
        Self::new()
    }
}
...
impl SharedState {
    ...
    /// Toggles the bit at the specified `position` in the [`SharedState`], including logs
    /// regarding the shared state of the memory after toggling. The `mode` parameter is used to
    /// differentiate between different states or operations (e.g., reading or writing) for
    /// debugging purposes.
    ///
