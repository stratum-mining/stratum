...
impl fmt::Display for RequestExtensions<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Clone to get the inner vec for display
        let extensions_list = format!("{:?}", self.requested_extensions.clone().into_inner());
        write!(
            f,
            "RequestExtensions(request_id: {}, requested_extensions: {})",
            self.request_id, extensions_list
        )
    }
}
...
impl fmt::Display for RequestExtensionsSuccess<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Clone to get the inner vec for display
        let extensions_list = format!("{:?}", self.supported_extensions.clone().into_inner());
        write!(
            f,
...
impl fmt::Display for RequestExtensionsError<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        // Clone to get the inner vecs for display
        let unsupported_list = format!("{:?}", self.unsupported_extensions.clone().into_inner());
        let required_list = format!("{:?}", self.required_extensions.clone().into_inner());
        write!(
            f,
            "RequestExtensions.Error(request_id: {}, unsupported: {}, required: {})",
            self.request_id, unsupported_list, required_list
        )
    }
}
