...
fn main() {
    ...
    let tlv_list_empty = TlvList::from_slice(&[]);
    ...
    let tlv_list_with = TlvList::from_slice(&[tlv]);
    ...
    for tlv in &tlvs {
        if tlv.type_.extension_type == EXTENSION_TYPE_WORKER_HASHRATE_TRACKING
        ...
        match result {
            Ok(tlv) => {
                println!(
                    ...
                    tlv.type_.extension_type, tlv.type_.field_type, tlv.length
                );
    ...
    println!(
        ...
        tlv.type_.extension_type, tlv.type_.field_type
    );
