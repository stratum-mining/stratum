...
use parsers_sv2::{
    parse_message_frame_with_tlvs, AnyMessage, CommonMessages, CommonMessagesOwned, Tlv,
};
...
pub trait HandleCommonMessagesFromServerOwnedSync {
    ...
    fn handle_common_message_frame_from_server(
        ...
        if negotiated_extensions.is_empty() {
            ...
            return self.handle_common_message_from_server(server_id, parsed.into_owned(), None);
        }
        let (message, tlv_fields) =
            parse_message_frame_with_tlvs(header, payload, &negotiated_extensions)
                .map_err(Self::Error::parse_error)?;
        match message {
            AnyMessage::Common(parsed) => self.handle_common_message_from_server(
                server_id,
                parsed.into_owned(),
                tlv_fields.as_deref(),
            ),
    ...
    fn handle_common_message_from_server(
        &mut self,
        server_id: Option<usize>,
        message: CommonMessagesOwned,
        tlv_fields: Option<&[Tlv]>,
    ) -> Result<(), Self::Error> {
        match message {
            CommonMessagesOwned::SetupConnectionSuccess(msg) => {
                self.handle_setup_connection_success(server_id, msg, tlv_fields)
            }
            CommonMessagesOwned::SetupConnectionError(msg) => {
                self.handle_setup_connection_error(server_id, msg, tlv_fields)
            }
            CommonMessagesOwned::ChannelEndpointChanged(msg) => {
                self.handle_channel_endpoint_changed(server_id, msg, tlv_fields)
            }
            CommonMessagesOwned::Reconnect(msg) => {
                self.handle_reconnect(server_id, msg, tlv_fields)
            }

            CommonMessagesOwned::SetupConnection(_) => Err(Self::Error::unexpected_message(
                0,
...
pub trait HandleCommonMessagesFromServerOwnedAsync {
    type Error: HandlerErrorType;
        ...
        async move {
            ...
            if negotiated_extensions.is_empty() {
                let parsed: CommonMessages<'_> = (header.msg_type(), payload)
                    ...
                    .map_err(Self::Error::parse_error)?;
                return self
                    .handle_common_message_from_server(server_id, parsed.into_owned(), None)
                    .await;
            }
            ...
            match parsed {
                AnyMessage::Common(parsed) => {
                    self.handle_common_message_from_server(
                        server_id,
                        parsed.into_owned(),
                        tlv_fields.as_deref(),
                    )
...
pub trait HandleCommonMessagesFromClientOwnedSync {
    type Error: HandlerErrorType;
    ...
    fn handle_common_message_frame_from_client(
        ...
        if negotiated_extensions.is_empty() {
            let parsed: CommonMessages<'_> = (header.msg_type(), payload)
                .try_into()
                .map_err(Self::Error::parse_error)?;
            return self.handle_common_message_from_client(client_id, parsed.into_owned(), None);
        }
        let (parsed, tlv_fields) =
            parse_message_frame_with_tlvs(header, payload, &negotiated_extensions)
                .map_err(Self::Error::parse_error)?;
        match parsed {
            AnyMessage::Common(parsed) => self.handle_common_message_from_client(
                client_id,
                parsed.into_owned(),
                tlv_fields.as_deref(),
            ),
    ...
    fn handle_common_message_from_client(
        ...
        match message {
            CommonMessagesOwned::SetupConnectionSuccess(_) => Err(Self::Error::unexpected_message(
                0,
                MESSAGE_TYPE_SETUP_CONNECTION_SUCCESS,
            )),
            CommonMessagesOwned::SetupConnectionError(_) => Err(Self::Error::unexpected_message(
                0,
                MESSAGE_TYPE_SETUP_CONNECTION_ERROR,
            )),
            CommonMessagesOwned::ChannelEndpointChanged(_) => Err(Self::Error::unexpected_message(
                0,
                MESSAGE_TYPE_CHANNEL_ENDPOINT_CHANGED,
            )),
            CommonMessagesOwned::Reconnect(_) => {
                Err(Self::Error::unexpected_message(0, MESSAGE_TYPE_RECONNECT))
            }

            CommonMessagesOwned::SetupConnection(msg) => {
...
/// returned by `get_negotiated_extensions_with_client()`.
#[trait_variant::make(Send)]
pub trait HandleCommonMessagesFromClientOwnedAsync {
    type Error: HandlerErrorType;
        ...
        async move {
            ...
            if negotiated_extensions.is_empty() {
                let parsed: CommonMessages<'_> = (header.msg_type(), payload)
                    ...
                    .map_err(Self::Error::parse_error)?;
                return self
                    .handle_common_message_from_client(client_id, parsed.into_owned(), None)
                    .await;
            }
            let (parsed, tlv_fields) =
                parse_message_frame_with_tlvs(header, payload, &negotiated_extensions)
            ...
            match parsed {
                AnyMessage::Common(parsed) => {
                    self.handle_common_message_from_client(
                        client_id,
                        parsed.into_owned(),
                        tlv_fields.as_deref(),
                    )
    ...
    async fn handle_common_message_from_client(
        ...
        async move {
            match message {
                CommonMessagesOwned::SetupConnectionSuccess(_) => Err(
                    Self::Error::unexpected_message(0, MESSAGE_TYPE_SETUP_CONNECTION_SUCCESS),
                ),
                CommonMessagesOwned::SetupConnectionError(_) => Err(
                    Self::Error::unexpected_message(0, MESSAGE_TYPE_SETUP_CONNECTION_ERROR),
                ),
                CommonMessagesOwned::ChannelEndpointChanged(_) => Err(
                    Self::Error::unexpected_message(0, MESSAGE_TYPE_CHANNEL_ENDPOINT_CHANGED),
                ),
                CommonMessagesOwned::Reconnect(_) => {
                    Err(Self::Error::unexpected_message(0, MESSAGE_TYPE_RECONNECT))
                }
