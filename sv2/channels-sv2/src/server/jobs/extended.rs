...
pub trait ExtendedJob<'a>: Job {
    // the standard job type that this extended job could be potentially converted to
    type StandardJob: StandardJob<'a>;
    ...
    fn into_standard_job(
        ...
        channel_id: u32,
        extranonce_prefix: Vec<u8>,
    ) -> Result<Self::StandardJob, ExtendedJobError>
    where
        Self: Sized;
...
impl<'a> ExtendedJob<'a> for DefaultExtendedJob<'a> {
    type StandardJob = DefaultStandardJob<'a>;
    ...
    /// Returns the coinbase transaction prefix for this job without BIP141 data.
    fn get_coinbase_tx_prefix_without_bip141(&self) -> Vec<u8> {
