...
Crates under `sv1`, `sv2` and `utils` workspaces follow SemVer 2.0.0. The version number is stored in
...
- If a release includes only bug fixes in `sv1`, `sv2` and/or `utils`, then `Z` is bumped.
- If a release includes breaking and/or non-breaking changes to `sv1`, `sv2` and/or `utils`, then `Y` is bumped.
- If a release marks a milestone i.e., `sv2` is reaching a new maturity level, then `X` is bumped.
