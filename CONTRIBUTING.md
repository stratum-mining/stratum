...
The only exception to the rules above is `stratum-core` crate: `PATCH` version bumps of `stratum-core` crate must be reserved to `PATCH` global releases of `stratum` repo.
